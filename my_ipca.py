...
class InstrumentedPCA(BaseEstimator):
    ...
    def predictOOS(self, X=None, y=None, indices=None, mean_factor=False, n=1):
        """
        ...
        # input x,y are from 1 to T = t+1
        X, y, indices, metad = _prep_input(X, y, indices)
        N, L, T = metad["N"], metad["L"], metad["T"]  # T=t+1
        y_returns = [np.full((N,), np.nan) for _ in range(n)]
        ...
        for t in range(T-n):
            ...
            X_t = X[ix,:]
            y_t2 = y[iy]
            array1 = indices[ix][:, 0].astype(int)
            array2 = indices[iy][:, 0].astype(int)
            results = np.intersect1d(array1, array2)
            
            bool_array = np.isin(array2, results)
            y_t2 = y_t2[bool_array]
            bool_array = np.isin(array1, results)
            X_t = X_t[bool_array]
    
            Numer = self.Gamma.T.dot(X_t.T).dot(y_t2)
            Denom = self.Gamma.T.dot(X_t.T).dot(X_t).dot(self.Gamma)

            Factor_OOS = np.linalg.solve(Denom, Numer.reshape((-1, 1)))
            
            
            if mean_factor:
                ypred[results] = np.ravel(X_t.dot(self.Gamma)\
                                 .dot(np.mean(self.Factors, axis=1).reshape((-1, 1))))
            else:
                ypred[results] = np.ravel(X_t.dot(self.Gamma).dot(Factor_OOS))
            
            y_returns.append(ypred)
